...
class Browser():
    def __init__(self,host, port, savedir,console,runtime):
    ...
    def new_tab(self,target_id):
        if not target_id in self.tab_dict.keys():
            ...
            self.tab_dict[target_id] = TabHandler(self.browser_host_port, target_id, self.callback, self.result_dir, self.dumplog, self.dumpfile,self.console,self.runtime)
...
class TabHandler():
    ...
    def __init__(self,browser_host_port, target_id, callback, result_dir,dumplog,dumpfile,console,runtime):
    ...
    def connect(self):
        ...
        try:
            ...
            param_list = [
                ...
                {"id":8,"method":"Page.startScreencast",
                    "params":{
                        "format":"png",
                        "quality":50,
                        "maxWidth":1024,
            ...
            if self.runtime:
                param_list.append({"id":9,"method":"Runtime.enable"})
                param_list.append({"id":10,"method":"Runtime.runIfWaitingForDebugger"})
                if self.console:
                    param_list.append({"id":11,"method":"Console.enable"})
                    param_list.append({"id":12,"method":"Runtime.discardConsoleEntries"})
                # this below may be useless now (?)
                #else:
                #    param_list.append({"id":8,"method":"Console.disable"})
            for message in param_list:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-nrt','--no-runtime',dest='runtime',action='store_false', help='disable Javascript Runtime.enable events and API do avoid CDP detection',default=True) 
    ...
    while "DevTools listening on" not in chromeout:
        chromeout= chromium.stderr.readline().decode()
    time.sleep(1)
    client = Browser(args.cdp_ip,args.cdp_port,args.savedir,args.console,args.runtime)
